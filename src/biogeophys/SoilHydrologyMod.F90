...
!#6
   ...
   subroutine LateralFlowPowerLaw(bounds, num_hydrologyc, filter_hydrologyc, &
     ...
     associate(                                                            &
       ...
       call lateral_outflow_inst%LateralOutflow(bounds, num_hydrologyc, filter_hydrologyc, &
            col, grc, soilhydrology_inst, soilstate_inst, &
            jwt = jwt(bounds%begc:bounds%endc), &
            dzmm = dzmm(bounds%begc:bounds%endc,:))

       call LateralInflow(bounds, num_hydrologyc, filter_hydrologyc, &
            num_hillslopec, filter_hillslopec, col, &
            qflx_latflow_out_vol = lateral_outflow_inst%qflx_latflow_out_vol_col(bounds%begc:bounds%endc), &
            qflx_latflow_in = qflx_latflow_in(bounds%begc:bounds%endc))
   ...
   !-----------------------------------------------------------------------
   subroutine LateralInflow(bounds, num_hydrologyc, filter_hydrologyc, &
        num_hillslopec, filter_hillslopec, col, &
        qflx_latflow_out_vol, qflx_latflow_in)
     !
     ! !DESCRIPTION:
     ! Compute lateral inflow - e.g., down the hillslope
     !
     ! Sets qflx_latflow_in everywhere within the hydrology filter
     !
     ! !ARGUMENTS:
     type(bounds_type) , intent(in)    :: bounds
     integer           , intent(in)    :: num_hydrologyc                      ! number of column soil points in column filter
     ...
     real(r8)          , intent(inout) :: qflx_latflow_in(bounds%begc: )      ! computed lateral inflow (mm/s)
     !
     ! !LOCAL VARIABLES:
     integer :: fc, c

     character(len=*), parameter :: subname = 'LateralInflow'
     !-----------------------------------------------------------------------

     SHR_ASSERT_ALL((ubound(qflx_latflow_out_vol) == (/bounds%endc/)), errMsg(sourcefile, __LINE__))
     SHR_ASSERT_ALL((ubound(qflx_latflow_in) == (/bounds%endc/)), errMsg(sourcefile, __LINE__))
     ...
     do fc = 1, num_hillslopec
        c = filter_hillslopec(fc)

        ! hilltop column has no inflow
        !
        ! TODO(wjs, 2018-01-24) I think this is redundant with the initialization above,
        ! and so can be removed. (The only way I could see this having any effect is if
        ! some column has qflx_latflow_in set to non-zero below then later zeroed here.
        ! The challenge of analyzing this is another motivation for the rework suggested
        ! in the below todo note.)
        ...
        !
        ! TODO(wjs, 2018-01-24) I'd find this a lot clearer if this algorithm were flipped
        ! around so that we're always setting qflx_latflow_in(c). I think this could be
        ! done by replacing indices of (c) with (col%colu(c)) and indices of
        ! (col%cold(c)) with (c). One reason this is better is: A filter loop should only
        ! set columns within that filter. Here we're setting col%cold(c), not c, and it's
        ! not obvious that there's a one-to-one correspondence between columns c in the
        ! hillslope filter and columns col%cold(c).
